...
class BlockchainWalletUI {
    ...
    connectWebSockets() {
        ...
        ws.onclose = () => {
            ...
            this.state.webSockets.node1 = null;
        ...
        div.innerHTML = `
            <div class="flex justify-between items-start">
                <div>
                    ...
                    ${transaction.message ? `<p class="text-xs text-gray-500 mt-1">${transaction.message}</p>` : ''}
                </div>
    ...
    updateTransactionStatus(transaction) {
        const transactionElements = document.querySelectorAll('.transaction-animation');
        transactionElements.forEach(element => {
            if (element.querySelector(`[data-tx-id="${transaction.id}"]`)) {
                const statusElement = element.querySelector('.status');
                if (statusElement) {
                    statusElement.textContent = '완료';
                    statusElement.className = 'text-xs text-green-600 mr-2';
                }
            }
        });
    }
...
window.addEventListener('load', () => {
    window.walletUI = new BlockchainWalletUI();
});